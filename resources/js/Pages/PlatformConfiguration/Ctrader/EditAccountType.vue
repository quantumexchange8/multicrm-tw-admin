...
const form = useForm({
    ...
    minimum_deposit: props.account.minimum_deposit,
    leverage: props.account.leverage,
    currency: props.account.currency,
    allow_create_account: -1,
    commission_structure: props.account.commission_structure,
    trade_open_duration: props.account.trade_open_duration,
});

const submit = () => {
    form.patch(route('platform_configuration.updateAccountType'), {
        onSuccess: () => {
            closeModal();
            form.reset();
        },
...
const emit = defineEmits(['update:accountTypeModal']);
const closeModal = () => {
    emit('update:accountTypeModal', false);
}
</script>

<template>
    <h2 class="text-lg mb-2 font-medium text-gray-900 dark:text-gray-100">{{ $t('public.Manage Account Type') }}</h2>
    <hr>
