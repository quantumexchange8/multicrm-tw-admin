...
const userTransactionHistory = ref({data: []});
const type = ref('');
const date = ref('');
...
const getResults = async (page = 1, user_id = props.user.id, type = '', date = '') => {
    ...
    try {
        let url = `/transaction/getCashWalletTransactionHistory/${user_id}?page=${page}`;
        ...
        userTransactionHistory.value = response.data;
    } catch (error) {
        console.error(error);
    } finally {
...
const getAmountClass = (history) => {
    if (history.type === 'WalletToAccount' || history.type === 'Withdrawal' || (history.type === 'WalletAdjustment' && history.amount < 0)) {
        return 'text-[#FF3F34]';
    } else if (history.type === 'AccountToWallet' || history.type === 'Deposit' || (history.type === 'WalletAdjustment' && history.amount >= 0)) {
        return 'text-[#05C46B]';
    }
    return '';
};
...
const getAmountPrefix = (history) => {
    if (history.type === 'WalletToAccount' || history.type === 'Withdrawal') {
        return '-';
    } else if (history.type === 'AccountToWallet' || history.type === 'Deposit' || (history.type === 'WalletAdjustment' && history.amount >= 0)) {
        return '+';
    }
...
<template>
    <div class="grid grid-cols-3 mt-8 gap-6">
        <div class="space-y-2">
            ...
            >
                ...
                <option value="Deposit">{{ $t('public.Deposit') }}</option>
                <option value="Withdrawal">{{ $t('public.Withdrawal') }}</option>
                ...
                <option value="WalletAdjustment">{{ $t('public.Wallet Adjustment') }}</option>
            </InputSelect>
        </div>
        <div class="space-y-2">
    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 mt-8" v-else>
        ...
        <tbody>
        <tr v-if="userTransactionHistory.data.length === 0">
            <th colspan="5" class="py-4 text-lg text-center">
                {{ $t('public.No History') }}
            </th>
        </tr>
        <tr v-for="history in userTransactionHistory.data" class="bg-white odd:dark:bg-transparent even:dark:bg-dark-eval-0 text-xs font-thin text-gray-900 dark:text-white text-center">
