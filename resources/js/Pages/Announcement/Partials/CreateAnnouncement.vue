...
<template>
    ...
    >
        {{ $t('public.Create New Announcement') }}
    </Button>

    <Modal :show="createAnnouncementModal" @close="closeModal" max-width="7xl">
        <div class="p-6">
            <h2 class="text-lg mb-2 font-medium text-gray-900 dark:text-gray-100">{{ $t('public.Create New Announcement') }}</h2>
            <hr>
            ...
            <form action="">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        <Label for="image">{{ $t('public.Upload Document') }}</Label>
                        <input
                            type="file"
                            id="image"
                            accept="image/*,.pdf"
                ...
                <div class="mt-6 flex gap-4 justify-end">
                    ...
                    <Button @click="submit" :disabled="form.processing">{{ $t('public.Create') }}</Button>
                </div>
            </form>
        </div>
    </Modal>
